...
var activeLanguageCode = "en-us";
var formControl = document.getElementById("date-value");
var activeLanguage = languages[activeLanguageCode];
var dist = {
    year: date.getFullYear(),
    month: date.getMonth(),
    date: date.getDate()
};
var startDate = new Date(dist.year, dist.month, dist.date);
var endDate = new Date(dist.year + 10, dist.month + 6, dist.date);
var options = {
    el: "#datepicker",
    startDate: startDate,
    endDate: endDate,
...
function createDatePicker(selected, options) {
    var app = null;
    app = new TypePicker(options);
    console.log(app);
    app.on("update", function (result) {
        if (result.type === "selected" && result.value.length === 2) {
            popupHandler(false);
        }
        formControl.value = result.value;
    });
    app.on("disabled", function (result) {
        var dateList = result.dateList, nodeList = result.nodeList;
    ...
    app.on("data", function (result) {
        var data = result.data;
        var nodeList = result.nodeList;
        for (var i = 0; i < nodeList.length; i++) {
            var node = nodeList[i];
            var date_1 = node.getAttribute("data-date");
            if (date_1 in data) {
                if (!node.classList.contains("disabled")) {
                    var itemData = source[date_1];
    ...
    app.setLanguage({
        title: function (year, month) { return activeLanguage["months"][month] + " " + year; },
