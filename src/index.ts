...
const getDisableDates = (startDate: Date,
                         endDate: Date,
                         dateFormat: string,
                         should: boolean) => {
    const temp: any = {};
...
function setNodeRangeState(el: HTMLElement,
                           data: Array<any>,
                           should?: boolean) {
    if (!should) return;
...
export default class DatePicker {
        ...
        nextTick(() => {
            ...
            this.disables = merge(
                getDisableDates(
                    ...
                    this.endDate,
                    this.dateFormat,
                    bindData || !isUndefined(this.startDate) || !isUndefined(this.endDate)
                ),
                disabledMap
            );
            //去掉data里的无效日期
            ...
            const prevDate = this.format(new Date(this.parse(peek).getTime() - (24 * 3600 * 1000))).value;
            if (
                this.disables[front] ||
                this.disables[peek] && this.disables[prevDate] ||
                (this.doubleSelect && front && front === peek && peek)
            ) {
                warn("setDates", "Illegal dates [" + this.selected + "]");
                this.selected = [];
            }
            console.log(this.disables[peek], this.disables[prevDate]);
            if (this.views === "auto") {
    ...
    constructor(option: datePickerOptions) {
        if (!option || !parseEl(option.el)) return;
