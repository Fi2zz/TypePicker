...
export default class DatePicker {
    ...
    private render(baseClassName: string, data: Array<any>, renderWeekOnTop: boolean) {
    ...
    constructor(option: datePickerOptions) {
        ...
        Observer.$on("render", (result: any) => {
            ...
            if (!isEmpty(this.disableDays) && (isUndefined(this.endDate) || isUndefined(this.startDate))) {
            ...
            for (let i = 0; i < nodeList.length; i++) {
                let node = nodeList[i];
                node.addEventListener("click", () => Observer.$emit("select", pickDate(node)));
            }
        });
        this.init(option);
    }
