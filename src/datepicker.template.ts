...
export default class HTML {
  constructor(options: classTemplate) {
    const { date, views, language, dateFormat, size } = options;

    this.dateFormat = dateFormat;
    const gap = views === 2 ? 1 : views === "auto" ? size : 0;
    this.language = language;
    this.views = views;
    // this.template = `${this.createActionBar(this.views !== 'auto')}${this.createView(this.createBody(gap, date))}`

    return <any>[
      `${this.createActionBar(this.views !== "auto")}${this.createView(
        this.createBody(gap, date)
      )}`
    ];
  }
  ...
  private createMonthWrap(head: string, body: string, week?: string) {
    return `<div class="calendar-main">${head} ${
      week ? week : ""
    } ${body}</div>`;
  }
